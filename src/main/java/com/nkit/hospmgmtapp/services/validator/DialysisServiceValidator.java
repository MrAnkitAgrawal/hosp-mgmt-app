...
public class DialysisServiceValidator {
  ...
  public void checkIfDialysisAlreadyScheduled(PatientE patientE, LocalDate scheduleDate) {
    Optional<DialysisScheduleE> scheduleE =
        dialysisScheduleR.findByScheduleDateAndStatusAndPatientEPatientId(
            scheduleDate, SCHEDULED, patientE.getPatientId());

    if (scheduleE.isPresent()) {
      throw new RuntimeException(DIALYSIS_ALREADY_SCHEDULED_FOR_THIS_PATIENT_ON_REQUESTED_DATE);
    }
  }
